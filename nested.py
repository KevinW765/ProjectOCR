...
for i in range(len(x_value)-1):
    for n in range(i+1, len(x_value)):
        if x_value[i] < x_value[n] < x2_value[n] < x2_value[i] and \
            ...
            table[i] = ET.SubElement(container, "tbody")
            table[i].set('class', 'table table'+str(i+1))
            table[n] = ET.SubElement(table[i], "tbody")
            table[n].set('class', 'table table'+str(n+1))
        elif x_value[n] < x_value[i] < x2_value[i] < x2_value[n] and \
                y_value[n] < y_value[i] < y2_value[i] < y2_value[n]:
            table[n] = ET.SubElement(container, "tbody")
            table[n].set('class', 'table table'+str(n+1))
            table[i] = ET.SubElement(table[n], "tbody")
            table[i].set('class', 'table table'+str(i+1))
        else:
            table[i] = ET.SubElement(container, "tbody")
            table[i].set('class', 'table table'+str(i+1))
